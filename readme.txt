...
исполнить подгрузив селери, функция написана, в админке надо только
выставить срок 3мин и убрать time.sleep, которые сейчас. селери сама будет считать минуты и выполнять таску.
...
Улучшение:
асинхронно заполнять таблицу, devider.py разделяет uszips.csv на много файлов, их надо вставить в табличку. Попытался
асинхронно. Вероятно, что-то асинхронное в файле fill_locations.py и появилось, но не все исполняется, как я рассчитывал
##########################
